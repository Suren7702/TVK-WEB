...
export default function AdminDashboard() {
  ...
  const [targetLevel, setTargetLevel] = useState(""); // union, village, ward, booth
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!token) { clearAuth(); navigate("/admin/login"); return; }
      setAuthToken(token);
      try {
        const { data } = await API.get("/auth/me");
        if (data.role !== "admin") { clearAuth(); navigate("/admin/login"); return; }
        setUser(data);
        ...
        clearAuth(); navigate("/admin/login");
      }
    };
    init();
  }, [navigate]);
  ...
  // --- DATA LOADERS ---
  const loadNews = async () => { try { const { data } = await API.get("/news"); setNewsList(data); } catch {} };
  const loadEvents = async () => { try { const { data } = await API.get("/events"); setEvents(data); } catch {} };
  const loadPartyNetwork = async () => {
    try {
      const { data } = await API.get("/party-network"); 
      setPartyNetwork(data); 
    } catch (err) {
      console.error("Failed to load party network", err);
    }
  };
  ...
  const handleNewsSubmit = async (e) => {
    e.preventDefault();
    try { await API.post("/news", newsForm); setNewsForm({ title: "", content: "", imageUrl: "", category: "district" }); loadNews(); } 
    catch (err) { alert("Error adding news"); }
  };
  const handleNewsDelete = async (id) => {
    if (!confirm("Delete this news?")) return;
    try { await API.delete(`/news/${id}`); loadNews(); } catch {}
  };
  ...
  const handleEventSubmit = async (e) => {
    e.preventDefault();
    try { await API.post("/events", eventForm); setEventForm({ title: "", description: "", imageUrl: "", date: "", location: "" }); loadEvents(); }
    catch (err) { alert("Error adding event"); }
  };
  const handleEventDelete = async (id) => {
    if (!confirm("Delete this event?")) return;
    try { await API.delete(`/events/${id}`); loadEvents(); } catch {}
  };
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
        // Limit file size to 2MB to prevent DB issues
        ...
        reader.onloadend = () => {
            // This sets the photoUrl to a "data:image/..." string
  ...
  const handleBarrierSubmit = async (e) => {
    ...
    try {
        if (isEditing) {
            await API.put(`/party-network/${editId}`, payload);
            setBarrierStatus("Updated Successfully! ✅");
        } else {
            await API.post("/party-network/add", payload);
  ...
  const handleBarrierDelete = async (nodeId, level) => {
      ...
      try {
          await API.delete(`/party-network/${nodeId}`, { data: { level } });
          loadPartyNetwork();
      } catch (err) {
